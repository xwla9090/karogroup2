  ...
  useEffect(() => {
    ...
    const doSync = async () => {
      try {
        ...
        var hash = exp.length + "" + conc.length + "" + loans.length + "" + contr.length + "" + inv.length + "" + cashIQD + "" + cashUSD + "" + uLen + "_" + Math.floor(Date.now()/2000);
    ...
    doSync();
    var interval = setInterval(doSync, 30000);
