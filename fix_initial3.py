...
old1 = """    // کاتی کرایەوەی براوزەر — تەنها ئەگەر localStorage بەتاڵە داتا بخوێنەوە
...
new1 = """    // کاتی کرایەوەی براوزەر — هەموو جار conc و exp بخوێنەوە، cash تەنها ئەگەر بەتاڵ بوو
    const initialLoad = async () => {
      const localCash = localStorage.getItem("karo_cashIQD_" + project);
...
if old1 in src:
    ...
    changes.append("✅ FIX 1: initialLoad — هەموو جار conc و exp بخوێنەوە")
else:
...
print("\n" + "="*55)
print("  کارۆ گروپ — InitialLoad Fix 2")
print("="*55)
for c in changes:
    print(c)
print("="*55)
...
print('  git add . && git commit -m "fix: initialLoad always fetch conc exp" && git push')
