...
# FIX 1: cash useEffect — cashRemoteRef زیاد بکە
old1 = """  useEffect(() => {
...
if old1 in src:
    ...
    changes.append("✅ FIX 1: cashRemoteRef زیاد کرا")
else:
    changes.append("⚠️  FIX 1: cash useEffect نەدۆزرایەوە")

# FIX 2: onCashUpdate — cashRemoteRef.current = true زیاد بکە
...
if old2 in src:
    ...
    changes.append("✅ FIX 2: onCashUpdate — cashRemoteRef.current = true زیاد کرا")
else:
...
print('  git add . && git commit -m "fix: cash remote ref loop" && git push')
